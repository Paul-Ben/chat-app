        ...
        if (chat.is_group) {
            ...
            if (otherParticipant) {
                ...
                if (otherParticipant.profile_pic) {
                    chatImage = `<img src="${otherParticipant.profile_pic}" alt="${otherParticipant.name}" class="small-profile-pic">`;
                } else {
...
function openChat(chat) {
    ...
    if (chat.is_group) {
        ...
        if (otherParticipant) {
            ...
            if (otherParticipant.profile_pic) {
                chatImage = `<img src="${otherParticipant.profile_pic}" alt="${otherParticipant.name}" class="profile-pic">`;
            } else {
                const initial = otherParticipant.name.charAt(0).toUpperCase();
...
function renderMessages() {
    ...
    Object.keys(groupedMessages).forEach(dateKey => {
        ...
        dateMessages.forEach(message => {
            ...
            // Format timestamp
            const msgTime = new Date(message.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
